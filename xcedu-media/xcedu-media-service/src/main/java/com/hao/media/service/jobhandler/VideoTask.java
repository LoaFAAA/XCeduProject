package com.hao.media.service.jobhandler;
...
public class VideoTask {
    ...
    public void shardingJobHandler() throws Exception {
        ...
        //查询待处理的任务
        List<MediaProcess> mediaProcessList = mediaProcessService.getMediaProcessList(shardTotal,shardIndex,processor);

        int TaskCount = mediaProcessList.size(); //任务数量
        //创建线程池
        ...
        mediaProcessList.forEach(mediaProcess -> {
            ...
            executorService.execute(() ->{
                ...
                //分配任务
                boolean acquireStatus = mediaProcessService.acpuireTask(taskId);
                if (!acquireStatus){
                    log.debug("未成功获取锁,id为：{}",taskId);
                    return;
                }
                //执行视频转码
